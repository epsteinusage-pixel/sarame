...
function library:create()
    ...
    function window:create_tab(name)
        ...
        local config_folder = "AnarchyConfigs"
        ...
        local function load_default_setting()
            if isfile(config_folder .. "/default.txt") then
                default_config = readfile(config_folder .. "/default.txt")
            end
        end
        
        local function save_default_setting(config_name)
            writefile(config_folder .. "/default.txt", config_name)
        ...
        local function get_configs()
            ...
            if isfolder(config_folder) then
                for _, file in pairs(listfiles(config_folder)) do
                    local name = file:gsub(config_folder .. "\\", ""):gsub(config_folder .. "/", ""):gsub(".json", "")
                    if name ~= "default.txt" then
        ...
        local function save_config(name)
            local file_path = config_folder .. "/" .. name .. ".json"
            writefile(file_path, HttpService:JSONEncode(config_data))
        ...
        local function load_config(name)
            ...
            if isfile(file_path) then
                ...
                config_data = HttpService:JSONDecode(data)
                current_config = name
        ...
        local function delete_config(name)
            ...
            if isfile(file_path) then
                ...
                if default_config == name then
                    default_config = nil
                    if isfile(config_folder .. "/default.txt") then
                        delfile(config_folder .. "/default.txt")
                    end
                end
        ...
        config_container.Name = "ConfigSystem"
        config_container.Size = UDim2.new(1, -15, 0, 280)
        config_container.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
        config_container.BorderSizePixel = 0
        config_container.Parent = page
        ...
        local config_name_input = Instance.new("TextBox")
        config_name_input.Size = UDim2.new(1, -24, 0, 30)
        config_name_input.Position = UDim2.new(0, 12, 0, 40)
        config_name_input.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        config_name_input.Text = ""
        config_name_input.PlaceholderText = "Config Name..."
        config_name_input.PlaceholderColor3 = Color3.fromRGB(120, 120, 120)
        config_name_input.TextColor3 = Color3.fromRGB(200, 200, 200)
        config_name_input.TextSize = 13
        config_name_input.Font = Enum.Font.Roboto
        config_name_input.TextXAlignment = Enum.TextXAlignment.Left
        config_name_input.ClearTextOnFocus = false
        config_name_input.Parent = config_container
    
        local input_corner = Instance.new("UICorner")
        input_corner.CornerRadius = UDim.new(0, 4)
        input_corner.Parent = config_name_input
    
        local input_stroke = Instance.new("UIStroke")
        input_stroke.Thickness = 1
        input_stroke.Color = Color3.fromRGB(60, 60, 60)
        input_stroke.Parent = config_name_input
    
        local input_padding = Instance.new("UIPadding")
        input_padding.PaddingLeft = UDim.new(0, 8)
        input_padding.PaddingRight = UDim.new(0, 8)
        input_padding.Parent = config_name_input
    
        local text_stroke = Instance.new("UIStroke")
        text_stroke.Thickness = 1.5
        text_stroke.Color = Color3.fromRGB(0, 0, 0)
        text_stroke.Parent = config_name_input
    
        local button_frame = Instance.new("Frame")
        button_frame.Size = UDim2.new(1, -24, 0, 140)
        button_frame.Position = UDim2.new(0, 12, 0, 80)
        button_frame.BackgroundTransparency = 1
        button_frame.Parent = config_container
    
        local button_layout = Instance.new("UIGridLayout")
        button_layout.CellSize = UDim2.new(0.48, 0, 0, 30)
        button_layout.CellPadding = UDim2.new(0.04, 0, 0, 8)
        button_layout.Parent = button_frame
    
        local function create_config_button(text, callback)
            local btn_container = Instance.new("Frame")
            btn_container.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
            btn_container.BorderSizePixel = 0
            btn_container.Parent = button_frame
    
            local btn_corner = Instance.new("UICorner")
            btn_corner.CornerRadius = UDim.new(0, 4)
            btn_corner.Parent = btn_container
    
            local btn_stroke = Instance.new("UIStroke")
            btn_stroke.Thickness = 0.5
            btn_stroke.Color = Color3.fromRGB(100, 100, 100)
            btn_stroke.Parent = btn_container
            ...
            local btn_text_stroke = Instance.new("UIStroke")
            btn_text_stroke.Thickness = 1.2
            btn_text_stroke.Color = Color3.fromRGB(0, 0, 0)
            btn_text_stroke.Parent = btn
    
            btn.MouseButton1Click:Connect(function()
                tween_service:Create(btn, TweenInfo.new(0.1), {TextColor3 = Color3.fromRGB(106, 152, 242)}):Play()
                tween_service:Create(btn_stroke, TweenInfo.new(0.1), {Color = Color3.fromRGB(106, 152, 242)}):Play()
                
                callback()
                
                task.wait(0.1)
                tween_service:Create(btn, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(200, 200, 200)}):Play()
                tween_service:Create(btn_stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(100, 100, 100)}):Play()
            end)
    
            btn.MouseEnter:Connect(function()
                tween_service:Create(btn_container, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 40)}):Play()
            end)
    
            btn.MouseLeave:Connect(function()
                tween_service:Create(btn_container, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}):Play()
            end)
    
            return btn_container
        end
    
        create_config_button("Create", function()
            local name = config_name_input.Text
            if name ~= "" then
                save_config(name)
                current_config = name
                config_name_input.Text = ""
        ...
        create_config_button("Save", function()
        ...
        create_config_button("Load", function()
            local name = config_name_input.Text
            if name ~= "" then
        ...
        create_config_button("Overwrite", function()
            if current_config then
                save_config(current_config)
            else
                print("No config selected")
        ...
        create_config_button("Delete", function()
            local name = config_name_input.Text
            if name ~= "" then
        ...
        create_config_button("Set Default", function()
            if current_config then
                save_default_setting(current_config)
                print("Set as default:", current_config)
            else
                print("No config selected")
            end
        end)
        ...
        status_label.Size = UDim2.new(1, -24, 0, 20)
        status_label.Position = UDim2.new(0, 12, 0, 230)
        status_label.BackgroundTransparency = 1
        status_label.Text = "Current: " .. (current_config or "None") .. " | Default: " .. (default_config or "None")
        status_label.TextColor3 = Color3.fromRGB(150, 150, 150)
        status_label.TextSize = 11
        status_label.Font = Enum.Font.Roboto
        status_label.TextXAlignment = Enum.TextXAlignment.Left
        status_label.Parent = config_container
    
        local status_stroke = Instance.new("UIStroke")
        status_stroke.Thickness = 1.2
        status_stroke.Color = Color3.fromRGB(0, 0, 0)
        ...
        config_name_input.Focused:Connect(function()
            tween_service:Create(input_stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(106, 152, 242)}):Play()
        end)
    
        config_name_input.FocusLost:Connect(function()
            tween_service:Create(input_stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(60, 60, 60)}):Play()
        end)
        ...
        return {
            save = function() if current_config then save_config(current_config) end end,
            load = function(name) return load_config(name) end,
            delete = function(name) return delete_config(name) end,
            get_current = function() return current_config end,
            set_data = function(key, value) config_data[key] = value end,
            get_data = function(key) return config_data[key] end,
